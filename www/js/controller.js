...
control.controller('loginCtrl', function($scope, $state, $ionicModal,$ionicSideMenuDelegate,$ionicScrollDelegate,$ionicLoading,$ionicPopup,$ionicPopover,$location,$http){
	$scope.userlogin = {};
...
control.controller('homeCtrl', function($scope, $state, $ionicModal,$ionicSideMenuDelegate, $ionicScrollDelegate,$ionicLoading,$ionicPopup,$ionicPopover,$location,$http,$ionicActionSheet){
            ...
            if(index == 2){
                ...
                navigator.screenshot.save(function(error,res){
                  if(error){
                    ...
                    console.log('ok',res.filePath); //should be path/to/myScreenshot.jpg
                    window.plugins.socialsharing.shareViaEmail(
                      dataPath.short_url, // can contain HTML tags, but support on Android is rather limited:  http://stackoverflow.com/questions/15136480/how-to-send-html-content-with-image-through-android-default-email-client
                      'MAB Files Backup',
                      [], // TO: must be null or an array
                      [], // CC: must be null or an array
                      null, // BCC: must be null or an array
                ...
                navigator.screenshot.save(function(error,res){
                  if(error){
                    ...
                    console.log('ok',res.filePath); //should be path/to/myScreenshot.jpg
                    window.plugins.socialsharing.shareViaTwitter(dataPath.short_url, [res.filePath], '');
                  }
                },'jpg',50,'myScreenShot');
                ...
                navigator.screenshot.save(function(error,res){
                  if(error){
                    ...
                    console.log('ok',res.filePath); //should be path/to/myScreenshot.jpg
                    window.plugins.socialsharing.shareViaFacebook(dataPath.short_url, [res.filePath], null /* url */, function() {console.log('share ok')}, function(errormsg){alert(errormsg)});
                  }
                },'jpg',50,'myScreenShot');
    ...
    $scope.folderEditFtn = function(){
        ...
        .success(function(data,status,headers,config){
            ...
            .error(function(data, status, headers, config){
                    ...
                    var alertPopup = $ionicPopup.alert({
                        title: 'MABFiles',
                        template: 'Login expired in MAB Files!'
                    });
                    alertPopup.then(function(res) {
                        $state.go('login');   
                    });
    ...
    $scope.filesCreateFtn = function(){
        ...
        if($('#newrootfiles').get(0).files[0]){
            $scope.filePath = 'Home/'+$('#newrootfiles').get(0).files[0].name;
            $ionicLoading.show({
                template: '<i class="icon ion-loading-a"></i>&nbsp;Please wait...'
            });

            var value = 0;
            for (var i = 0; i < $('#newrootfiles').get(0).files.length; i++) {
                var formData = new FormData();
                formData.append("api_key",loginStorage.api_key);
                formData.append("file", $('#newrootfiles').get(0).files[i]);
                ...
                .success(function(data, status, headers, config){
                    value = value+1;
                    console.log(value+" : "+ $('#newrootfiles').get(0).files.length);
                    if(value == $('#newrootfiles').get(0).files.length ){
                        $http({method:'GET', url:'http://build.myappbuilder.com/api/files.json', cache:false, params:{'api_key':loginStorage.api_key}})
                        .success(function(data, status, headers, config){
                            //alert(JSON.stringify(data));
                            $("#newrootfiles").val('');
...
control.controller('filesListCtrl', function($scope, $state, $ionicModal,$ionicSideMenuDelegate, $ionicPopover, $ionicScrollDelegate,$ionicLoading,$ionicPopup,$location,$http,$ionicActionSheet){
    ...
    $scope.filesCreateFtn = function(){
        //formData.append("file", $('#newfiles').get(0).files[0]);
        if($('#newfiles').get(0).files[0]){

            $scope.filePath = 'Home/'+$scope.homeFileName+'/'+$('#newfiles').get(0).files[0].name;

            $ionicLoading.show({
                template: '<i class="icon ion-loading-a"></i>&nbsp;Please wait...'
            });
            var value = 0;
            for (var i = 0; i < $('#newfiles').get(0).files.length; i++) {
                var formData = new FormData();
                formData.append("api_key",loginStorage.api_key);
                formData.append("folder_id",folderDetails.id);
                formData.append("file", $('#newfiles').get(0).files[i]);
                $http.post('http://build.myappbuilder.com/api/files/new.json',formData,{
                    transformRequest: angular.identity,
                    headers: {'Content-Type': undefined}
                })
                .success(function(data, status, headers, config){
                    value = value+1;
                    if(value == $('#newfiles').get(0).files.length ){
                        $http({method:'GET', url:'http://build.myappbuilder.com/api/files.json', cache:false, params:{'api_key':loginStorage.api_key,'folder_id':folderDetails.id}})
                        .success(function(data, status, headers, config){
                            $('#newfiles').val('');
                            $ionicLoading.hide();
...
control.controller('subfilesListCtrl', function($scope, $state, $ionicModal,$ionicSideMenuDelegate, $ionicPopover, $ionicScrollDelegate,$ionicLoading,$ionicPopup,$location,$http,$ionicActionSheet){
    ...
    $scope.filesCreateFtn = function(){
        //formData.append("file", $('#newfiles').get(0).files[0]);
        if($('#newfiles').get(0).files[0]){
            $scope.filePath = 'Home/'+$scope.homeFileName+'/'+folderFlieName+'/'+$('#newfiles').get(0).files[0].name;
            $ionicLoading.show({
                template: '<i class="icon ion-loading-a"></i>&nbsp;Please wait...'
            });
            var value = 0;
            for (var i = 0; i < $('#newfiles').get(0).files.length; i++) {
                ...
                .success(function(data, status, headers, config){
                    value = value+1;
                    console.log(value+" : "+ $('#newfiles').get(0).files.length);
                    if(value == $('#newfiles').get(0).files.length ){
                        $http({method:'GET', url:'http://build.myappbuilder.com/api/files.json', cache:false, params:{'api_key':loginStorage.api_key,'folder_id':subfolderDetails.id}})
                        .success(function(data, status, headers, config){
                            $("#newfiles").val('');
...
control.controller('subfilesList1Ctrl', function($scope, $state, $ionicModal,$ionicSideMenuDelegate, $ionicPopover, $ionicScrollDelegate,$ionicLoading,$ionicPopup,$location,$http,$ionicActionSheet){
    ...
    $scope.filesCreateFtn = function(){
        //formData.append("file", $('#newfiles').get(0).files[0]);
        if($('#newfiles').get(0).files[0]){
            $scope.filePath = 'Home/'+$scope.homeFileName+'/'+folderFlieName+'/'+folderFlieName1+'/'+$('#newfiles').get(0).files[0].name;
            $ionicLoading.show({
                template: '<i class="icon ion-loading-a"></i>&nbsp;Please wait...'
            });
            var value = 0;
...
control.controller('subfilesList2Ctrl', function($scope, $state, $ionicModal,$ionicSideMenuDelegate, $ionicPopover, $ionicScrollDelegate,$ionicLoading,$ionicPopup,$location,$http,$ionicActionSheet){
    ...
    $scope.filesCreateFtn = function(){
        //formData.append("file", $('#newfiles').get(0).files[0]);
        if($('#newfiles').get(0).files[0]){
            $scope.filePath = 'Home/'+$scope.homeFileName+'/'+folderFlieName+'/'+folderFlieName1+'/'+folderFlieName2+'/'+$('#newfiles').get(0).files[0].name;
            $ionicLoading.show({
                template: '<i class="icon ion-loading-a"></i>&nbsp;Please wait...'
            });
            var value = 0;
